...
int main()
{
  ...
  long double  pon,c,inf, sup, z,r,t,x,u1,u2,u0,al,pic, time;
 
  const  double d= 1;

// The parameters a1,a2 and a3 for the fuzzy number c, and moment (time).
cout<<" Enter the lower range, the peak and the upper range of the triangular membership, and the time: "<<endl;
cin>>inf; cin>>pic; cin>>sup, cin>>time;
...
t=time;
	   ...
	   x=40;  //Initial point in the pipe.
	 while(x<=80)   // Length of the whole, in this case (<80)
       	{         
          al=0;
          while (al<=0.9)
		    {
